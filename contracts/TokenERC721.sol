...
interface ITokenERC721 is IERC721Upgradeable {
  ...
  event TokensMintedWithSignature(
    address indexed signer, address indexed mintedTo, uint256 indexed tokenIdMinted, MintRequest mintRequest
  );
  ...
  NFTMetadata
{
  ...
  function mintWithSignature(
    ...
    if (_req.royaltyRecipient != address(0)) {
      royaltyInfoForToken[tokenIdMinted] = RoyaltyInfo({recipient: _req.royaltyRecipient, bps: _req.royaltyBps});
    }
  ...
  function verifyRequest(MintRequest calldata _req, bytes calldata _signature) internal returns (address) {
    ...
    require(
      _req.validityStartTimestamp <= block.timestamp && _req.validityEndTimestamp >= block.timestamp, 'request expired'
    );
